...
static void decode_buffer(u8 index) {
    u32 s; s16 pcm;
    for(s=0; s<GDAA_STREAM_BUFFER_SAMPLES; s++) {
        if(gdaa_next_sample(&stream_decoder, &pcm)) {
            stream_buffers[index][s] = gdaa_pcm16_to_pcm8(pcm);
        } else {
            stream_buffers[index][s] = 0;
        }
    }
}

static void arm_dma2(u8 index) {
    REG_DMA2CNT = 0;
    REG_DMA2SAD = (u32)stream_buffers[index];
    REG_DMA2DAD = (u32)&REG_FIFOB;
    REG_DMA2CNT = DMA_ENABLE | DMA_START_FIFO | DMA_REPEAT | DMA_32BIT | DMA_DEST_FIXED;
}

void gdaa_stream_timer_irq(void) {
    u8 next, rec;
    if(!stream_playing || stream_paused) return;
    next = dma_buffer_index ^ 1;
...
void gdaa_stream_init(void) {
    REG_SNDSTAT |= DS_MASTER_ENABLE;
    REG_SNDDSCNT |= DS_B_VOLUME_100 | DS_B_RIGHT | DS_B_LEFT | DS_B_TIMER1 | DS_B_FIFO_RESET;
    REG_DMA2CNT = 0;
    REG_TM1CNT = 0;
    REG_TM2CNT = 0;
...
void gdaa_stream_resume(void) {
    if(!stream_playing || !stream_paused) return;
    decode_buffer(dma_buffer_index);
    arm_dma2(dma_buffer_index);
    REG_TM2D = TIMER_RELOAD(GDAA_STREAM_BUFFER_SAMPLES - FIFO_AHEAD_SAMPLES);
    REG_TM2CNT = TIMER_ENABLE | TIMER_CASCADE | TIMER_IRQ;
    REG_TM1CNT = TIMER_ENABLE;
    stream_paused = FALSE;
